...
players_df = players_df.withColumn("playername", regexp_extract(col("player"), r"^([^-]+)", 1))
players_df = players_df.withColumn("SRT", regexp_extract(col("player"), r"-(.+)$", 1))
players_df = players_df.withColumn("50s", regexp_extract(col("50s/100s"), r"^(\d+)", 1))
players_df = players_df.withColumn("100s", regexp_extract(col("50s/100s"), r"/(\d+)$", 1))
players_df = players_df.withColumn("sum", expr("CAST(`50s` AS INT) + CAST(`100s` AS INT)"))
